...
import { getRFQQuotations, acceptQuotation, rejectQuotation, Quotation } from '@/lib/api/quotation'
import { useRouter } from 'next/navigation'
import Link from 'next/link'

export default function RFQQuotationsPage({ params }: { params: { id: string } }) {
  ...
  const loadQuotations = async () => {
    try {
      const data = await getRFQQuotations(parseInt(params.id))
      setQuotations(data)
    } catch (err) {
        ...
        <div className="sm:flex sm:items-center">
          <div className="sm:flex-auto">
            ...
            <p className="mt-2 text-sm text-gray-700">
              Review and manage quotations for your RFQ
            </p>
          </div>
                ...
                {quotations.length === 0 ? (
                  <div className="text-center py-12 bg-white">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Wait for suppliers to submit their quotations.
                    </p>
                  </div>
                ) : (
                      ...
                      <div key={quotation.id} className="border-b border-gray-200 p-6">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="mt-1 text-sm text-gray-500">
                              Submitted {new Date(quotation.created_at).toLocaleDateString()}
                            </p>
                          </div>
                        ...
                        <dl className="mt-4 grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2 lg:grid-cols-3">
                          ...
                          {quotation.technical_specifications && (
                            <div className="sm:col-span-2 lg:col-span-3">
                        ...
                        {quotation.status === 'submitted' && (
                          <div className="mt-6 flex space-x-3">
                            ...
                            >
                              {processingId === quotation.id ? 'Processing...' : 'Accept'}
                            </button>
                            <button
                              type="button"
                            ...
                            >
                              {processingId === quotation.id ? 'Processing...' : 'Reject'}
                            </button>
                          </div>
                        )}
                      </div>
