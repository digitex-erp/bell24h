...
export const usePaymentProcessing = ({ onSuccess, onError }: UsePaymentProcessingProps = {}) => {
  ...
  const {
    createPaymentIntent: createPaymentIntentContext,
    confirmPayment: confirmPaymentContext,
    processing,
    error,
    setError,
  } = usePayment();
  ...
  const createPaymentIntent = useCallback(async (
    ...
    try {
      ...
      const paymentIntent = await createPaymentIntentContext(amount, currency, metadata);
  ...
  }, [createPaymentIntentContext, enqueueSnackbar, onError]);
  ...
  const confirmPayment = useCallback(async (
    ...
    try {
      ...
      const success = await confirmPaymentContext(paymentMethodId, savePaymentMethod);
  ...
  }, [confirmPaymentContext, currentPaymentIntent, enqueueSnackbar, onError, onSuccess]);
