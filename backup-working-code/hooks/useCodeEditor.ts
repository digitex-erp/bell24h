...
export const useCodeEditor = ({
  ...
  const updateCode = useCallback((newCode: string, triggerAnalysis: boolean = autoAnalyze) => {
    setCode(newCode);
  ...
  const generateTests = useCallback(async (testFramework: 'jest' | 'mocha' | 'jasmine' | 'pytest' = 'jest') => {
    try {
      setIsProcessing(true);
  ...
  const refactorCode = useCallback(async (goals: Array<'performance' | 'readability' | 'security' | 'best-practices'> = ['best-practices']) => {
    try {
      setIsProcessing(true);
  ...
  const generateDocumentation = useCallback(async (format: 'jsdoc' | 'tsdoc' | 'markdown' = 'jsdoc') => {
    try {
      setIsProcessing(true);
