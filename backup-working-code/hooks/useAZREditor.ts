...
export const useAZREditor = ({
  ...
  const handleCodeChange = useCallback((newCode: string, cursorPos: number = 0) => {
    setCode(newCode);
  ...
  // Analyze code
  const analyzeCode = useCallback(async (codeToAnalyze: string = code) => {
  ...
  const getCodeCompletions = useCallback(async (prefix: string, suffix: string = '') => {
    setIsProcessing(true);
    try {
      const context = {
  ...
  // Generate tests
  const generateTests = useCallback(async (testFramework?: string, options: any = {}) => {
    setIsProcessing(true);
    try {
      const framework = testFramework || config.TEST_GENERATION.DEFAULT_FRAMEWORK;
      const result = await generateTestsService(
        code,
        framework as any,
        { 
          ...options,
          language: language || contextLanguage,
          includeDescriptions: options.includeDescriptions ?? config.TEST_GENERATION.INCLUDE_DESCRIPTIONS,
  ...
  // Refactor code
  const refactorCode = useCallback(async (goals?: string[]) => {
    setIsProcessing(true);
    try {
  ...
  const generateDocumentation = useCallback(async (format?: string, options: any = {}) => {
    setIsProcessing(true);
    try {
      const docFormat = format || config.DOCUMENTATION.DEFAULT_FORMAT;
      const result = await generateDocumentationService(
        code,
        docFormat as any,
        {
          ...options,
          ...
          includeParamDescriptions: options.includeParamDescriptions ?? config.DOCUMENTATION.INCLUDE_PARAM_DESCRIPTIONS,
  ...
  // Apply a completion
  const applyCompletion = useCallback((completionText: string) => {
