...
export function useEscrow(): UseEscrowReturn {
  ...
  const toggleEscrow = useCallback(async (walletId: string, isEnabled: boolean) => {
  ...
  const updateEscrowThreshold = useCallback(async (walletId: string, threshold: number) => {
  ...
  const createEscrowHold = useCallback(async (data: {
    walletId: string;
    amount: number;
    currency: string;
    orderId?: string;
  ...
  const releaseEscrow = useCallback(async (escrowHoldId: string, metadata: any = {}) => {
  ...
  const refundEscrow = useCallback(async (escrowHoldId: string, metadata: any = {}) => {
