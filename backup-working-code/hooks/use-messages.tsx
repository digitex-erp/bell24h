  ...
  const sendMessageMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      if (variables.recipient_id) {
        queryKeys.push(`/api/messages?user_id=${variables.recipient_id}`);
      }
      
      if (variables.related_rfq_id) {
        queryKeys.push(`/api/messages?rfq_id=${variables.related_rfq_id}`);
      }
      
      if (variables.related_bid_id) {
        queryKeys.push(`/api/messages?bid_id=${variables.related_bid_id}`);
      }
  ...
  const markAsReadMutation = useMutation({
    ...
    onSuccess: (updatedMessage) => {
      ...
      if (updatedMessage.sender_id) {
        queryKeys.push(`/api/messages?user_id=${updatedMessage.sender_id}`);
      }
      
      if (updatedMessage.related_rfq_id) {
        queryKeys.push(`/api/messages?rfq_id=${updatedMessage.related_rfq_id}`);
      }
      
      if (updatedMessage.related_bid_id) {
        queryKeys.push(`/api/messages?bid_id=${updatedMessage.related_bid_id}`);
      }
