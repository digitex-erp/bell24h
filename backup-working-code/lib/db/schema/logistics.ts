...
import { pgTable, serial, varchar, text, timestamp, json, enum as pgEnum } from 'drizzle-orm/pg-core';
...
export const shipmentUpdates = pgTable('shipment_updates', {
  id: serial('id').primaryKey(),
  shipmentId: serial('shipment_id').references(() => shipments.id, { onDelete: 'cascade' }).notNull(),
