...
# Bell24H SSL Setup Script - Automated
# This script installs Cloudflare Origin Certificate and configures Nginx for HTTPS
...
sudo tee /etc/ssl/certs/cloudflare/origin.crt > /dev/null <<EOF
$CERT
EOF

# Step 3: Save private key
echo "🔑 Saving private key..."
sudo tee /etc/ssl/certs/cloudflare/origin.key > /dev/null <<EOF
$KEY
EOF
...
# Step 5: Update bell24h Nginx config
...
# Step 6: Update n8n Nginx config
