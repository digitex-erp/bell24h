# Bell24H SSL Setup - Manual Steps

Since automated execution isn't working, here are the **exact copy-paste commands** to run manually.

## Step 1: SSH into your VM

Open PowerShell or Command Prompt and run:

```bash
ssh -i "C:\Users\Sanika\Downloads\oracle-ssh-bell\ssh-key-2025-10-01.key" ubuntu@80.225.192.248
```

---

## Step 2: Create SSL Directory

Once connected to the VM, run:

```bash
sudo mkdir -p /etc/ssl/certs/cloudflare
```

---

## Step 3: Save Certificate

Run this command (it will open nano editor):

```bash
sudo nano /etc/ssl/certs/cloudflare/origin.crt
```

**In nano:**
1. **Right-click** to paste this certificate:
```
-----BEGIN CERTIFICATE-----



-----END CERTIFICATE-----
```

2. Press **Ctrl+O**, then **Enter** to save
3. Press **Ctrl+X** to exit

---

## Step 4: Save Private Key

Run:

```bash
sudo nano /etc/ssl/certs/cloudflare/origin.key
```

**In nano:**
1. **Right-click** to paste this private key:
```
-----BEGIN PRIVATE KEY-----



-----END PRIVATE KEY-----
```

2. Press **Ctrl+O**, then **Enter** to save
3. Press **Ctrl+X** to exit

---

## Step 5: Set Permissions

```bash
sudo chmod 600 /etc/ssl/certs/cloudflare/origin.key
sudo chmod 644 /etc/ssl/certs/cloudflare/origin.crt
```

---

## Step 6: Update bell24h Nginx Config

```bash
sudo nano /etc/nginx/sites-available/bell24h
```

**Delete everything** in the file (Ctrl+K repeatedly), then **paste this entire block**:

```nginx
# HTTPS Server Block (Port 443)
server {
    listen 443 ssl;
    server_name bell24h.com www.bell24h.com app.bell24h.com;

    ssl_certificate /etc/ssl/certs/cloudflare/origin.crt;
    ssl_certificate_key /etc/ssl/certs/cloudflare/origin.key;
    ssl_protocols TLSv1.2 TLSv1.3;
    ssl_ciphers HIGH:!aNULL:!MD5;

    location / {
        proxy_pass http://localhost:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
    }
}

# HTTP Server Block (Port 80) - Redirect to HTTPS
server {
    listen 80;
    server_name bell24h.com www.bell24h.com app.bell24h.com;
    return 301 https://$host$request_uri;
}
```

**Save:** Ctrl+O, Enter, Ctrl+X

---

## Step 7: Update n8n Nginx Config

```bash
sudo nano /etc/nginx/sites-available/n8n
```

**Delete everything** in the file, then **paste this entire block**:

```nginx
# HTTPS Server Block (Port 443)
server {
    listen 443 ssl;
    server_name n8n.bell24h.com;

    ssl_certificate /etc/ssl/certs/cloudflare/origin.crt;
    ssl_certificate_key /etc/ssl/certs/cloudflare/origin.key;
    ssl_protocols TLSv1.2 TLSv1.3;
    ssl_ciphers HIGH:!aNULL:!MD5;

    location / {
        proxy_pass http://localhost:5678;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
    }
}

# HTTP Server Block (Port 80) - Redirect to HTTPS
server {
    listen 80;
    server_name n8n.bell24h.com;
    return 301 https://$host$request_uri;
}
```

**Save:** Ctrl+O, Enter, Ctrl+X

---

## Step 8: Test and Reload Nginx

```bash
sudo nginx -t
sudo systemctl reload nginx
```

You should see:
```
nginx: the configuration file /etc/nginx/nginx.conf syntax is ok
nginx: configuration file /etc/nginx/nginx.conf test is successful
```

---

## Step 9: Test Your Sites

Wait 30 seconds, then open in your browser:

- ✅ https://bell24h.com
- ✅ https://www.bell24h.com
- ✅ https://app.bell24h.com
- ✅ https://n8n.bell24h.com

**You should now see:**
- ✅ **Green lock** (SSL working)
- ✅ **No 521 error**
- ✅ **Your app/n8n loading**

---

## 🎉 **DONE!**

Your sites are now **fully live with HTTPS**!

Reply once you've completed these steps or if you encounter any errors.

