...
// Global export
class Bell24hNotifications {
  constructor() {
    this.audio = new Audio();
    this.notificationCount = 0;
    this.sounds = {
      rfq: '/sounds/rfq-notification.mp3',
      bid: '/sounds/bid-update.mp3',
      payment: '/sounds/payment-processed.mp3',
      message: '/sounds/message-received.mp3',
      delivery: '/sounds/delivery-update.mp3',
      verification: '/sounds/verification-complete.mp3'
    };
  }

  async playSound(type) {
    this.audio.src = this.sounds[type] || this.sounds.message;
    try {
  ...
  async notify(title, message, type = 'message') {
    ...
    // Add notification to list
    this.addNotificationToList(title, message, type);
  }
...
if (typeof window !== 'undefined') {
  window.bell24hNotifications = new Bell24hNotifications();
}

export default Bell24hNotifications;
class Bell24hNotifications {
    constructor() {
        this.audio = new Audio();
        this.notificationCount = 0;
    ...
    async playSound(type) {
        ...
        this.audio.src = sounds[type] || sounds['message'];
        await this.audio.play();
    }
