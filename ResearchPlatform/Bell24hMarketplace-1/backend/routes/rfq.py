...
from backend.utils.ipfs_client import IPFSClient
...
async def create_rfq(rfq_data: RFQCreate, current_user = Depends(get_current_user)):
    ...
    try:
        ...
        # Store RFQ in blockchain
        blockchain_response = await web3_client.create_rfq(rfq_db_data)
        
        # Store documents in IPFS if any
        if rfq_data.documents:
            document_hashes = []
            for doc in rfq_data.documents:
                ipfs_hash = await ipfs_client.store_document(doc)
                document_hashes.append(ipfs_hash)
                # Verify document on blockchain
                await web3_client.verify_document(ipfs_hash)
            rfq_db_data['document_hashes'] = document_hashes
...
from backend.ai.bid_predictor import BidPricePredictor

bid_predictor = BidPricePredictor()

@router.post("/predict-price", response_model=Dict[str, Any])
async def predict_bid_price(
    rfq_data: Dict[str, Any],
    current_user = Depends(get_current_user)
):
    """Predict price range for an RFQ"""
    try:
        # Get historical bids
        historical_bids = supabase.table("bids").select("*").eq("category", rfq_data["category"]).execute()
        
        if not historical_bids.data:
            return {
                "min_price": rfq_data["budget"] * 0.8,
                "max_price": rfq_data["budget"] * 1.2,
                "confidence": 0.5,
                "message": "Limited historical data available"
            }
            
        prediction = bid_predictor.predict_price_range(rfq_data, historical_bids.data)
        return {
            **prediction,
