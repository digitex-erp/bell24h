...
def match_suppliers_for_rfq(rfq: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    This is a demo version that uses pre-defined suppliers and 
    implements the matching algorithm directly
    """
    # Extract RFQ details
    ...
    # Define feature weights (same as in the actual system)
    weights = np.array([
        0.25,  # Category match
    ...
    for supplier in SUPPLIERS:
        ...
        features = np.array([
            ...
            supplier.get("completion_rate", 0.0),  # Completion rate (0 to 1)
            delivery_match,  # Delivery time match (0 to 1)
            min(1.0, supplier.get("rating", 0.0) / 5.0),  # Rating (0 to 1)
            min(1.0, supplier.get("total_completed", 0) / 100.0)  # Experience level (0 to 1)
        ])
        
        feature_vectors.append(features)
        supplier_details.append(supplier)
    ...
    # Calculate mean features across all suppliers (for SHAP values)
    mean_features = np.mean(features_array, axis=0)
    
    # Calculate SHAP values for each supplier
    result = []
    for i, (supplier, score, features) in enumerate(zip(supplier_details, scores, features_array)):
        # Calculate SHAP values
        shap_values = (features - mean_features) * weights
        
        # Prepare explanations
        explanations = []
        for j, (name, value) in enumerate(zip(feature_names, shap_values)):
        ...
        # Sort explanations by absolute magnitude
        explanations.sort(key=lambda x: abs(x["value"]), reverse=True)
        
        # Add to result
        supplier_with_score = supplier.copy()
        supplier_with_score["match_score"] = float(score)
        supplier_with_score["shap_values"] = explanations
        result.append(supplier_with_score)
