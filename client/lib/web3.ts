...
const CONTRACT_ADDRESSES = {
  bellToken: '0x...', // Replace with actual deployed address
  bellEscrow: '0x...', // Replace with actual deployed address
  bellVerification: '0x...', // Replace with actual deployed address
};
...
interface Web3ContextType {
  ...
  contracts: {
    bellToken: ethers.Contract | null;
    bellEscrow: ethers.Contract | null;
  };
  error: string | null;
}
...
export const Web3Provider = ({ children }: Web3ProviderProps) => {
  ...
  const checkConnection = async (web3Provider: ethers.providers.Web3Provider) => {
    try {
  ...
  const connect = async () => {
    ...
    try {
      ...
      if (network.chainId !== POLYGON_CONFIG.chainId) {
        await switchNetwork();
      }
    } catch (err: any) {
  ...
  const contracts = {
    bellToken: signer ? new ethers.Contract(CONTRACT_ADDRESSES.bellToken, BELL_TOKEN_ABI, signer) : null,
    bellEscrow: signer ? new ethers.Contract(CONTRACT_ADDRESSES.bellEscrow, BELL_ESCROW_ABI, signer) : null,
  };
...
export const parseUnits = (amount: string, decimals: number = 18) => {
  return ethers.utils.parseUnits(amount, decimals);
};

export const formatUnits = (amount: string, decimals: number = 18) => {
...
export const useBellToken = () => {
  ...
  const stake = async (amount: string, duration: number) => {
    ...
    const tx = await contracts.bellToken.stake(parseUnits(amount), duration);
...
export const useBellEscrow = () => {
  ...
  const createEscrow = async (supplier: string, milestones: number[], amounts: string[]) => {
    ...
    const parsedAmounts = amounts.map(amount => parseUnits(amount));
