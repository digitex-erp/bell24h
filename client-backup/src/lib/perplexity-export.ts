...
async function exportToCsv(data: PerplexityExportData, filename: string): Promise<void> {
  ...
  if (data.additionalData) {
    ...
    if (data.additionalData.temporalTrends?.length > 0) {
...
async function exportToExcel(data: PerplexityExportData, filename: string): Promise<void> {
  ...
  if (data.perplexityResult) {
    ...
    if (data.perplexityResult.readabilityIndex) {
      overviewSheet.addRow({ property: 'Readability Index', value: data.perplexityResult.readabilityIndex });
    }
  }
...
async function exportToPdf(data: PerplexityExportData, filename: string): Promise<void> {
  ...
  if (data.perplexityResult) {
    ...
    if (data.perplexityResult.readabilityIndex) {
      perplexityResultTable.push(['Readability Index', data.perplexityResult.readabilityIndex.toString()]);
    }
    ...
    // Add perplexity factors if available
    if (data.perplexityResult.perplexityFactors?.length > 0) {
      docDefinition.content.push({ text: 'Perplexity Factors', style: 'subheader', margin: [0, 15, 0, 5] });
      
      const factorsTableBody: any[][] = [
        [{ text: 'Factor', style: 'tableHeader' }, { text: 'Impact', style: 'tableHeader' }]
      ];
  ...
  if (data.additionalData) {
    ...
    if (data.additionalData.successPrediction) {
      ...
      docDefinition.content.push({ text: 'Success Prediction', style: 'subheader', margin: [0, 15, 0, 5] });
      docDefinition.content.push({
        table: {
          headerRows: 0,
          widths: ['50%', '50%'],
      ...
      // Add areas chart
      if (sp.areas?.length > 0) {
        docDefinition.content.push({ text: 'Success Areas', style: 'subheader', margin: [0, 15, 0, 5] });
        
        const areasTableBody: any[][] = [
          [{ text: 'Area', style: 'tableHeader' }, { text: 'Score', style: 'tableHeader' }]
        ];
    ...
    // Temporal Trends
    if (data.additionalData.temporalTrends?.length > 0) {
      docDefinition.content.push({ text: 'Temporal Trends', style: 'subheader', margin: [0, 15, 0, 5] });
...
export async function generateCustomReport(
  ...
  try {
    ...
    if (reportResponse && reportResponse.pdfBase64) {
      ...
      // Save the file
      const reportFilename = filename || `perplexity-report-${data.entityType}-${new Date().getTime()}.pdf`;
