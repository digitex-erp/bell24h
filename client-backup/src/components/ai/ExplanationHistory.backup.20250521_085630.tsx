...
export const ExplanationHistory: React.FC<ExplanationHistoryProps> = ({
  ...
  availableModelTypes: propAvailableModelTypes = [],
  onPageChange,
  onSortChange,
  onSearch,
  onCompare,
  ...
  // State management
  const [availableModelTypes, setAvailableModelTypes] = useState<string[]>(propAvailableModelTypes);
  ...
  const [rowsPerPage, setRowsPerPage] = useState<number>(externalPageSize);
  const [sortBy, setSortBy] = useState<keyof ExtendedModelExplanation>('timestamp');
  ...
  const [selectedModelType, setSelectedModelType] = useState<string>(externalSelectedModelType);
  ...
  const filteredExplanations = useMemo(() => {
    ...
    if (selectedModelType) {
      result = result.filter(exp => exp.modelType === selectedModelType);
    }
    ...
    return result;
  }, [explanations, searchTerm, selectedModelType, sortBy, sortOrder]);
  
  // Handle export
  const handleExport = useCallback(async (format: ExportFormat, explanation: ExtendedModelExplanation) => {
    ...
    try {
      ...
      if (!exportElement) {
        throw new Error('Could not find explanation element to export');
      }
      ...
      switch (format) {
        case 'pdf':
          await exportToPdf(exportElement, baseFilename);
          success = true;
          break;
        case 'png':
          await exportToPng(exportElement, baseFilename);
          success = true;
          break;
        case 'csv':
      ...
      if (success) {
        setSnackbar({
          open: true,
          message: `Successfully exported explanation as ${format.toUpperCase()}`,
          severity: 'success'
        });
      }
    } catch (error) {
      ...
      setSnackbar({
        open: true,
        message: `Failed to export explanation: ${error instanceof Error ? error.message : 'Unknown error'}`,
        severity: 'error'
      });
    } finally {
      setExporting(false);
  ...
  const handleRowClick = useCallback((explanation: ExtendedModelExplanation) => {
    ...
    if (expandedExplanationId === explanation.id) {
      setExpandedExplanationId(null);
      if (onExplanationCollapse) {
        onExplanationCollapse(explanation);
      }
    } else {
      setExpandedExplanationId(explanation.id);
      if (onExplanationExpand) {
        onExplanationExpand(explanation);
      }
    }
  }, [expandedExplanationId, onExplanationClick, onExplanationCollapse, onExplanationExpand]);
  
  // Handle selection change
  const handleSelect = useCallback((explanation: ExtendedModelExplanation, selected: boolean) => {
    const newSelection = new Set(selectedForComparison);
    
    if (selected) {
      newSelection.add(explanation.id);
      if (onExplanationSelect) onExplanationSelect(explanation);
    } else {
      newSelection.delete(explanation.id);
      if (onExplanationDeselect) onExplanationDeselect(explanation);
    }
    ...
    if (externalOnSelectionChange) {
      externalOnSelectionChange(Array.from(newSelection));
    }
  }, [selectedForComparison, onExplanationSelect, onExplanationDeselect, onSelectionChange, externalOnSelectionChange]);
  ...
  // Handle sort
  const handleSort = useCallback((property: keyof ExtendedModelExplanation) => {
...
ExplanationHistory.defaultProps = {
  ...
  availableModelTypes: [],
  maxItems: 100,
} as Partial<ExplanationHistoryProps>;
