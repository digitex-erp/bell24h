...
import { VolumeUp, PlayArrow, VolumeOff } from '@mui/icons-material';
...
const AudioTestPage: React.FC = () => {
  ...
  const [showUnsupportedWarning, setShowUnsupportedWarning] = useState<boolean>(false);

  // Check if Web Audio API is supported
  useEffect(() => {
    if (typeof window === 'undefined') return;
    
    // @ts-ignore - Check for Web Audio API support
    const isSupported = !!(window.AudioContext || window.webkitAudioContext);
    setIsAudioSupported(isSupported);
    setShowUnsupportedWarning(!isSupported);
  }, []);
