...
function VoiceAnalyticsPage() {
  ...
  // Language name mapping (must be inside component to use t)
  const languageNames: Record<string, string> = {
    'en': t('languages.en', 'English'),
    'hi': t('languages.hi', 'Hindi'),
    'bn': t('languages.bn', 'Bengali'),
    'ta': t('languages.ta', 'Tamil'),
    'te': t('languages.te', 'Telugu'),
    'mr': t('languages.mr', 'Marathi'),
    'gu': t('languages.gu', 'Gujarati'),
    'kn': t('languages.kn', 'Kannada'),
    'ml': t('languages.ml', 'Malayalam'),
    'pa': t('languages.pa', 'Punjabi'),
    'others': t('languages.others', 'Others'),
  };
    ...
    return (
      <MainLayout>
        <div className="flex items-center justify-center min-h-[60vh]">
          ...
          <span className="ml-2">{t('voiceAnalytics.loading', 'Loading voice analytics data...')}</span>
        </div>
      </MainLayout>
    );
  }
    ...
    return (
      <MainLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
          ...
          <h2 className="text-2xl font-bold mb-2">{t('voiceAnalytics.errorTitle', 'Unable to load voice analytics')}</h2>
          <p className="text-muted-foreground mb-4">
            {error instanceof Error ? error.message : t('voiceAnalytics.unexpectedError', 'An unexpected error occurred')}
          </p>
      ...
      <div className="container mx-auto py-6">
        ...
        <Tabs defaultValue="overview" className="w-full">
          <TabsList className="grid grid-cols-5 mb-4">
            <TabsTrigger value="overview">{t('voiceAnalytics.tabs.overview', 'Overview')}</TabsTrigger>
            <TabsTrigger value="trend">{t('voiceAnalytics.tabs.trend', 'Trend')}</TabsTrigger>
            <TabsTrigger value="language">{t('voiceAnalytics.tabs.language', 'Language')}</TabsTrigger>
            <TabsTrigger value="quality">{t('voiceAnalytics.tabs.quality', 'Quality')}</TabsTrigger>
            <TabsTrigger value="submissions">{t('voiceAnalytics.tabs.submissions', 'Submissions')}</TabsTrigger>
          </TabsList>
          
          <TabsContent value="overview" className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle>{t('voiceAnalytics.voiceRfqSubmissionsOverTime', 'Voice RFQ Submissions Over Time')}</CardTitle>
                <CardDescription>
          ...
          <TabsContent value="language" className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle>{t('voiceAnalytics.detailedLanguageAnalysis', 'Detailed Language Analysis')}</CardTitle>
                <CardDescription>
                  {t('voiceAnalytics.languageBreakdown', 'Breakdown of language usage in voice RFQ submissions')}
                </CardDescription>
              </CardHeader>
              <CardContent>
                {/* More detailed language analysis would go here */}
                <p className="text-center text-muted-foreground py-8">
                  {t('voiceAnalytics.languageAnalysisSoon', 'Detailed language analysis will be available soon')}
                </p>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="quality" className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle>{t('voiceAnalytics.advancedQualityMetrics', 'Advanced Quality Metrics')}</CardTitle>
                <CardDescription>
                  {t('voiceAnalytics.qualityMetricsDescription', 'Detailed audio quality and processing metrics')}
                </CardDescription>
              </CardHeader>
              <CardContent>
                {/* More detailed quality metrics would go here */}
                <p className="text-center text-muted-foreground py-8">
                  {t('voiceAnalytics.qualityMetricsSoon', 'Advanced quality metrics will be available soon')}
                </p>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="submissions" className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle>{t('voiceAnalytics.recentSubmissions', 'Recent Voice RFQ Submissions')}</CardTitle>
                <CardDescription>
                  {t('voiceAnalytics.latestRfqsDescription', 'Latest voice-based RFQs with transcription and metadata')}
                </CardDescription>
              </CardHeader>
              <CardContent>
                {latestRfqs.length > 0 ? (
                  ...
                  <p className="text-center text-muted-foreground py-8">
                    {t('voiceAnalytics.noRecentSubmissions', 'No recent voice RFQ submissions found')}
                  </p>
                )}
              </CardContent>
              {latestRfqs.length > 0 && (
                <CardFooter>
                  <Button variant="outline" className="w-full">{t('voiceAnalytics.viewAllSubmissions', 'View All Submissions')}</Button>
                </CardFooter>
              )}
            </Card>
          </TabsContent>
